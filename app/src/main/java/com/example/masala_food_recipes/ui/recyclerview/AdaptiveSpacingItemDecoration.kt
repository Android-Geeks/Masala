...
/**
 * This class is used to add spacing between items in RecyclerView. It's adaptive, so it can be used with any
 * layout manager. It's also flexible, so it can be used with any span count. It's also customizable, so it can
 * be used with any spacing size.
 *
 * @param size        Spacing size between items
 * @param edgeEnabled Whether spacing is enabled on edge or not
 */
